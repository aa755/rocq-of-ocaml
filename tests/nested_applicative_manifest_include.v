...
Module Base.
  ...
  Module Wrapped.
    ...
    Definition Inner_include `{_fargs : FArgs} :
      Inner.Inner_result (_fargs := Inner_include_fargs) := (Inner.functor
      (_fargs := (Inner_include_fargs ))).
    
    (** Inclusion of the module [Inner_include] *)
    Definition t `{_fargs : FArgs} (a : Set) :=
      (Inner_include (_fargs := _fargs)).(Inner.Inner_result.t) a.
    
    Definition make `{_fargs : FArgs} {A : Set} :
      Value.(RocqOfOCaml.OCamlMap.OrderedType.t) -> A -> t A :=
      (Inner_include (_fargs := _fargs)).(Inner.Inner_result.make).
