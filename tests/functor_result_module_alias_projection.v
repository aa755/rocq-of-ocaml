...
Module State.
  ...
  Definition functor `{_fargs : FArgs} :@State_result _fargs :=
    ({|
      State_result.Inner_return (_fargs := _fargs) :=
        (Inner._return (_fargs := _fargs))
    |} : @State_result _fargs).
End State.
...
Module Inner.
  ...
  Module Inner_result.
    Record signature `{_fargs : FArgs} {StorageKey_Set_t : Set} : Set := {
  ...
  Definition functor `{_fargs : FArgs} :Inner_result (StorageKey_Set_t := _) :=
    {|
      Inner_result.StorageKey_Set := (StorageKey._Set (_fargs := _fargs));
      Inner_result.accessed_keys := (accessed_keys (_fargs := _fargs));
      Inner_result.M := (M (_fargs := _fargs));
      Inner_result.Nested := (Nested (_fargs := _fargs))
    |}.
End Inner.
...
Module Outer.
  ...
  Module Outer_result.
    Record signature `{_fargs : FArgs} {Host_StorageKey_Set_t : Set} : Set := {
      Host :
  ...
  Definition functor `{_fargs : FArgs}
    ...
    {|
      Outer_result.Host := (Host (_fargs := _fargs))
    |}.
End Outer.
...
Module Reexport.
  ...
  Module Reexport_result.
    Record signature `{_fargs : FArgs}
      {Instantiation_Host_StorageKey_Set_t : Set} : Set := {
      Instantiation :
  ...
  Definition functor `{_fargs : FArgs}
    ...
    {|
      ...
      Reexport_result.Host_accessed_keys :=
        (Host.accessed_keys (_fargs := _fargs));
      Reexport_result.Host_M_Inner_return :=
        (Host.M.Inner._return (_fargs := _fargs));
