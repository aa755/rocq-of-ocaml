...
let to_coq (imports : MonadEval.Import.t list) (ast : t) : SmartPrint.t =
  let uses_well_founded_recursion =
    ...
    | SignatureAxioms _ -> false
    | Structure structure ->
        Structure.uses_well_founded_recursion structure
  in
  !^"(** File generated by rocq-of-ocaml *)"
  ^^ newline
  ^^
  (if ast.without_default_imports then empty
   else
     concat
       (List.map
          (fun d -> d ^^ newline)
  ...
  ^^
  (if uses_well_founded_recursion then
     !^"From Stdlib Require Import Program.Wf." ^^ newline
   else empty)
