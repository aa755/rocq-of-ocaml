...
type t = { category : Category.t; loc : Loc.t; message : string }

let to_comment (error_message : string) : SmartPrint.t =
  !^("(* ❌ " ^ List.hd (String.split_on_char '\n' error_message) ^ " *)")

let pad (width : int) (character : char) (message_left : string)
    (message_right : string) : string =
...
let colorize_line (content : string) (loc : Loc.t) (line_number : int) : string
    =
  let left_limit =
    if line_number < loc.start.line then String.length content
    else if line_number = loc.start.line then loc.start.character - 1
    else 0
  in
  let right_limit =
    if line_number < loc.end_.line then String.length content
    else if line_number = loc.end_.line then loc.end_.character - 1
    else 0
  in
  colorize "33" (String.sub content 0 (left_limit - 0))
  ^ colorize "33;1" (String.sub content left_limit (right_limit - left_limit))
  ^ colorize "33"
      (String.sub content right_limit (String.length content - right_limit))
...
let get_code_frame (source_lines : string list) (loc : Loc.t) : string =
  ...
  for current_line_number = first_line_number to last_line_number do
    ...
    if current_line_index >= 0 && current_line_index < nb_source_lines then
      let current_line_content = List.nth source_lines current_line_index in
      let is_error_line =
        loc.start.line <= current_line_number
        && current_line_number <= loc.end_.line
      in
      let line_head = if is_error_line then colorize "31;1" "> " else "  " in
      let line_number =
        colorize
          (if is_error_line then "1" else "0")
          (pad line_number_width ' ' "" (string_of_int current_line_number)
          ^ " | ")
      in
      let line_content =
        colorize_line current_line_content loc current_line_number
      in
      let current_line = line_head ^ line_number ^ line_content in
      output_lines :=
        colorize (if is_error_line then "1" else "") current_line
        :: !output_lines
  done;
  String.concat "\n" (List.rev !output_lines)

let display_error (file_name : string) (source_lines : string list)
    (loc : Loc.t) (category : Category.t) (message : string)
    (error_number : int) (number_of_errors : int) : string =
  colorize "34;1"
    (pad 100 '-'
       ("--- " ^ file_name ^ ":"
       ...
       ^ " ")
       (" " ^ Category.to_id category ^ " (" ^ string_of_int error_number ^ "/"
       ^ string_of_int number_of_errors
       ^ ")" ^ " ---"))
  ^ "\n" ^ "\n"
  ^ get_code_frame source_lines loc
  ^ "\n" ^ "\n\n" ^ message ^ "\n\n"

let display_errors_human (source_file_name : string)
    (source_file_content : string) (errors : t list) : string =
...
let display_errors (json_mode : bool) (source_file_name : string)
    (source_file_content : string) (errors : t list) : string =
  let errors =
    errors
